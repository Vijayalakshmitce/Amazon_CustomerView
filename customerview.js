    ...
    inquirer.prompt([{
        ...
        connection.query("SELECT * FROM products_custmer where item_id = '" + answer.userProductID + "'", function (error, productresult) {
            ...
            if ((productresult[0].stock_quantity !== null) && (productresult[0].stock_quantity > 0)) {
                ...
                connection.query("UPDATE products_custmer SET stock_quantity = '" + totalProductQuantity + "' , Sold_item = '" + sold_item + "' WHERE item_id = '" + answer.userProductID + "'", function (err, res) {
                    if (err) throw err;
                });
